# RetrainNewAct/NewActRetrain.py
import os
import sys
import argparse
import torch
...
# Allow running from either project root or RetrainNewAct folder
...
def reset_loader_vocab(loader):
    """
    Full retraining reset:
    rebuild vocab / label space from the expanding window.
    """
...
def get_prefix_events_from_df(df: pd.DataFrame) -> set:
    events = set()
...
def get_model_known_events(loader) -> set:
    """
    Current semantic known event/activity space.

    Since the model has separate input token vocab and output label vocab,
    we use their union for reporting unseen target / unseen event statistics.
    """
...
def get_model_known_input_tokens(loader) -> set:
    """
    Current input token space.

    This is used to decide whether a prefix can be encoded and predicted.
    """
...
def split_predictable_df_like_simulation(batch_df: pd.DataFrame, known_input_tokens: set):
    """
    Simulation-style filtering.

    If the observed prefix contains an activity unknown to the current model's
    input space, no prediction is made for that sample (#NP).

    The target next_act is NOT used to decide predictability.
    If next_act is unknown but the prefix is known, the sample is still predicted
    and usually counted as wrong.
    """
    predictable_mask = []
    not_predicted_mask = []
    ...
    for _, row in batch_df.iterrows():
        ...
        prefix_known = all(tok in known_input_tokens for tok in prefix_tokens)
...
def get_new_prefix_activities(batch_df: pd.DataFrame, known_input_tokens: set) -> set:
    """
    New activity trigger for S2 / Retrain(new act).

    We follow the observed-sequence logic:
    a new activity is detected if it appears in the observed prefix
    but is not known to the current model input space.
    """
    new_acts = set()

    for prefix in batch_df["prefix"].astype(str).tolist():
        for tok in prefix.split():
            if tok not in known_input_tokens:
                new_acts.add(tok)
...
def train_fresh_model_on_df(args, loader, train_df, device):
    ...
    This intentionally does NOT reuse old weights.
    """
...
def main():
    ...
    ap.add_argument(
        "--excel_path",
        type=str,
        default="./RetrainNewAct/runs/new_act_retrain_window_metrics.xlsx",
    )
    ...
    for i, (win_key, batch_df) in enumerate(test_batches.items(), start=1):
        ...
        # New activities in observed prefixes trigger retraining after this window.
        new_activities_in_window = get_new_prefix_activities(
            batch_df=batch_df,
            known_input_tokens=known_input_tokens_before_window,
        )
        has_new_activity = len(new_activities_in_window) > 0
        ...
        print(
            f"[NewAct Check] has_new_activity={has_new_activity} "
            f"new_activities={sorted(list(new_activities_in_window))}"
        )
        ...
        window_records.append({
            ...
            "correct_predicted": int(correct_predicted),

            "acc": float(acc_predicted_only),
            "acc_predicted_only": float(acc_predicted_only),
            "acc_all_samples": float(acc_all_samples),
            ...
            "has_new_activity": bool(has_new_activity),
            "new_activity_count": int(len(new_activities_in_window)),
            "new_activities": ",".join(sorted(list(new_activities_in_window))),
        ...
        # ===== Retrain(new act): update only if unseen observed activities appeared =====
        if has_new_activity:
            retrain_count += 1

            print(
                f"\n[Retrain(new act)] after window={win_key} | "
            ...
            print(
                f"[Retrain(new act)] new known_events={len(known_events)} | "
    ...
    try:
        ...
        window_records.append({
            ...
            "window_id": "overall",
            "n_samples": int(total_samples_all),
            "predicted_count": int(total_predicted_count),
            "not_predicted_count": int(total_not_predicted_count),
            "correct_predicted": int(total_correct_predicted),

            "unseen_count": None,
            "unseen_ratio": None,
            "unseen_event_ratio": None,

            "acc": float(overall_acc_predicted_only),
            "acc_predicted_only": float(overall_acc_predicted_only),
            "acc_all_samples": float(overall_acc_all_samples),
    ...
    ckpt_path = os.path.join(args.out_dir, f"{args.dataset}_new_act_retrain.pt")
    vocab_path = os.path.join(args.out_dir, f"{args.dataset}_new_act_retrain_vocab.json")
